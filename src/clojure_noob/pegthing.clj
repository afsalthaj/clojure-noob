...
;; symbols
;; For now, all you need to know is that when you give Clojure a symbol like map, it finds the corresponding var in the current namespace, gets a shelf address, and retrieves an object from that shelf for you—in this case, the function that map refers to. If you want to just use the symbol itself, and not the thing it refers to, you have to quote it. Quoting any Clojure form tells Clojure not to evaluate it but to treat it as data. The next few examples show what happens when you quote a form.
...
;; By defining this function you are updating the maps with a new association, that is between `new-function` and var. The namespace will staore `afsal thaj`, get the address of the shelf and store it in a var.

somefun
;; return the var (#user/new-function)


;; so to repeat, the namespace will be a set of maps, consisting of association between object names and the var, where the address of the implementation of the object isstored. So in the above case the map will be:
...
;; in summary
;; given a symbol -> find the corresponding var in the map of ns (ns-interns if they are internal functions) -> get the shelf number from the var -> go to the shelf and retrieve the actual data/value


;; If you define a def with the same functon name, then the var has been updated with the address of the new vector. It’s like you used white-out on the address on a card in the card catalog and then wrote a new address. The result is that you can no longer ask Clojure to find the first vector. This is referred to as a name collision. Chaos! Anarchy!
...
;; An example of `require` and `refer`
...
;; This shows that file svg.clj exists in the directory visualization in the `the-divine-cheese-code` directory. This
;; directory exists in the root folder which is `src` when you create a lein project.
;; not that it is all symbol. It says, find the file that corresponds to this symbol using the rules described in “The Relationship Between File Paths and Namespace Names” on page 133. In this case, Clojure finds src/the_divine_cheese_code/visualization/svg.clj.

;; require also lets you alias a namespace when you require it, using :as or alias. This:

;; (require '[thenamespacedirectory.filenamewithoutclj :as svg]. In this case, you may call the defns and defs in the namespace as `svg/something`
...
;; that is you don't need to require it first and then refer. You may do it 
